...
function ExpenseForm({ onAddExpense }) {
  ...
  const validate = (fields = {}) => {
    ...
    if (!vals.amount || isNaN(vals.amount) || +vals.amount <= 0) next.amount = 'Enter a valid positive amount.';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await Promise.resolve(onAddExpense(newExpense)); // supports sync or async
      setSuccess(true);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-semibold text-indigo-300">Add New Expense</h3>
